...
func _build_cable(segment, joint, endA, endB):
	...
	var prev_v_position = -SEGMENT_LENGTH * GLOBAL_SCALE
	...
	for s in range(number_of_segments):
		...
		# place segment
		var seg_v_pos = prev_v_position + (SEGMENT_LENGTH * GLOBAL_SCALE) - nudge
		seg_copy.position = Vector2(0, seg_v_pos)
		prev_v_position = seg_v_pos

		# place joint
		# var joint_v_pos = s * (SEGMENT_LENGTH * GLOBAL_SCALE) - (nudge * s)
		# joint_copy.position = Vector2(0, joint_v_pos)
		joint_copy.position = Vector2(0, seg_v_pos)
	...
	$EndB.position = Vector2(0, end_v_position - nudge)

	# create one more joint to attach EndB to last segment
	var last_joint = joint.duplicate()
	last_joint.position = Vector2(0, end_v_position)
