...
var single_unit_width: int = hp_width / single_units_per_hp
...
var AnchorPoint = preload("res://AnchorPoint.gd")
var anchors = []
...
func _ready() -> void:
	...
	for i in temp_unit_total:
		...
		add_child(anchor)
	print("there are ", anchors.size(), " anchors")
...
func _input(event):
	...
	if placing_module_instance:
		...
		for anchor in _candidate_anchors:
			var dist = mouse_pos.distance_to(anchor.global_position)
...
func _load_module(module_scene: PackedScene, width: int, anchor) -> void:
	...
	# Optionally create test connection if this is the first module
	#if loaded_modules.size() == 1:
		#_create_test_connection(module_instance)
